...
class SiteController extends Controller
{
    ...
    public function actionRequest()
    {
       ...
       if($prod_attr_query == '' && $searchword == ''){
           $data = Yii::$app->cache->get('first'.$cat.'/'.$hideman.'/'.$start_price.'/'.$end_price.'/'.$count.'/'.$start_arr);
           if ($data === false) {
               ...
               Yii::$app->cache->set('first'.$cat.'/'.$hideman.'/'.$start_price.'/'.$end_price.'/'.$count.'/'.$start_arr, ['productattrib' => $productattrib, 'count_arrs' => $count_arrs, 'price_max' =>  $price_max, 'data' => $data ], 3600);
           }else{
