    ...
    public function actionRequest()
    {
       ...
       if(!isset($x[products_last_modified])){
           ...
           if ($data === false) {
               ...
               Yii::$app->cache->set(urlencode('two'.$cat_start.'-'.$hide_man.'-'.$start_price.'-'.$end_price.'-'.$count.'-'.$start_arr.'-'.$prod_attr_query.'-'.$sort), ['productattrib' => $productattrib, 'count_arrs' => $count_arrs, 'price_max' =>  $price_max, 'data' => $data ], 86400);
           }else{
               $productattrib = $data[productattrib];
               $count_arrs = $data[count_arrs];
               $price_max = $data[price_max];
