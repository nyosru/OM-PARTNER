...
trait ActionRegisterSP
{
    public function actionRegisterSp()
    {
        ...
        if (!Yii::$app->user->isGuest) {
            $userinfo = PartnersUsersInfo::find()->where(['id' => Yii::$app->getUser()->identity->getId()])->one()
                ->toArray()
            ;
            if (Yii::$app->request->post('accept')
                ...
                if (
                    $customer
                    && ($check = Referrals::find()->where(['user_id' => $userinfo['id']])
                        ->orWhere(['customer_id' => $customer['customers_id']])->exists()) == false
                ) {
                    ...
                    if ($refferal->validate() && $refferal->save()) {
                        ...
                        Yii::$app->mailer->compose('register-sp',
                            ['refer' => $refferal->referral_url, 'ident' => $refferal->id])
