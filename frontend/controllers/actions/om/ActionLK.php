...
trait ActionLK
{
    ...
    {
        if (Yii::$app->user->isGuest || ($cust = User::find()->where(['partners_users.id' => Yii::$app->user->getId(), 'partners_users.id_partners' => Yii::$app->params['constantapp']['APP_ID']])->joinWith('userinfo')->joinWith('customers')->joinWith('addressBook')->one()) == FALSE ) {
            return $this->redirect(Yii::$app->request->referrer);
        }


        if(isset($cust['customers']['customers_id'])){
            $this->layout = 'lk';
            ...
            $this->layout = 'lksp';
            $model = \common\models\PartnersOrders::find()->where(['user_id' => $cust['id']])->groupBy('orders.`orders_id` DESC');
