        ...
        <div id="main-index">
            ...
            <div id="main-spec">
                ...
                foreach($dataproducts as $value){
                    ...
                    $product['products_image'] = str_replace(' ','[[[[]]]]', $product['products_image']);
                    $product['products_image'] = str_replace('(','[[[[', $product['products_image']);
                    $outer .= '<div  class="container-fluid float" id="index-card-1" product=""><div data-prod="' . $product['products_id'] . '" id="prod-data-img"  style="clear: both; min-height: 180px; min-width: 200px; background-size:cover; background: no-repeat scroll 50% 50% / contain url(/site/imagepreview?src=' . $product['products_image'] . ');"></div><div class="name">' . $description['products_name'] . '</div><div class="model">Арт.' . $product['products_model'] . '</div><div class="price"><b>' . ((integer)($product['products_price']) + (integer)$product['products_price'] / 100 * (integer)(Yii::$app->params['partnersset']['discount']['value'])) . '</b> руб.</div><a href="/site/product?id=' . $product['products_id'] . '"><div id="prod-info" data-prod="' . $product['products_id'] . '">Инфо</div></a><span>' . $attr_html . '</span></div>';
                    echo $outer;
                }
                ?>
            </div>
