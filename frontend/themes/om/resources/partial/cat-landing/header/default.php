<div class="col-1">
    <div class="header_bar" style="height: 55px;border-bottom: 1px solid rgb(204, 204, 204);">
        <div class="header_bar_img">
            <img src="/images/logo/OM_code.png">
        </div>
    </div>
</div>

<div class="special_header">
    <h1><?= ($special_header) ?: 'Специальное предложение' ?></h1>
</div>

<div class="row" style="margin-top: 10px">
    <div class="col-1-4">
        <img style="display: block;max-width: 100%;height: auto; margin-bottom: 4px"
             src="/images/banners/OM_14122016_1.png" alt="Нарядный образ на работу">
        <img style="display: block;max-width: 100%;height: auto; " src="/images/banners/OM_14122016_4.png"
             alt="Наряд для маленькой принцессы">
    </div>
    <div class="col-1-4">
        <img style="display: block;max-width: 100%;height: auto; margin-bottom: 9px"
             src="/images/banners/OM_14122016_3.png" alt="Теплые колготки для зимнего образа">
        <img style="display: block;max-width: 100%;height: auto; " src="/images/banners/OM_14122016_2.png"
             alt="Уютные кофточки">
    </div>
    <div class="col-1-2">
        <img style="display: block;max-width: 100%;height: auto;" src="/images/banners/OM_14122016_5.png"
             alt="В новый год на корпоратив">
    </div>
</div>

<div class="row">
    <div class="col-1" style="margin-top: 10px">
        <a href="https://www.instagram.com/odezhda_master/" target="_blank"><img
                    style="display: block;max-width: 100%;height: auto;" src="/images/banners/OM_14122016_6.png"
                    alt="Топ 5 лучших товаров каждый день"></a>
    </div>
</div>

<div class="row" style="padding: 15px; border-bottom: 1px solid rgb(204, 204, 204);">
    <div class="col-1-5" style="margin-top: 10px">
        <div class="row">
            <div class="col-1-3">
                <img width="100%" src="/images/logo/low_price.png">
            </div>
            <div class="col-2-3 about_text">
                Пожалуй, самые низкие цены
            </div>
        </div>
    </div>
    <div class="col-1-5" style="margin-top: 10px">
        <div class="row">
            <div class="col-1-3">
                <img width="100%" src="/images/logo/orders.png">
            </div>
            <div class="col-2-3 about_text">
                Нами выполненно более 1,000,000 заказов
            </div>
        </div>
    </div>
    <div class="col-1-5" style="margin-top: 10px">
        <div class="row">
            <div class="col-1-3">
                <img width="100%" src="/images/logo/vipolneno.png">
            </div>
            <div class="col-2-3 about_text">
                Более 25,000 товаров для вашего выбора
            </div>
        </div>
    </div>
    <div class="col-1-5" style="margin-top: 10px">
        <div class="row">
            <div class="col-1-3">
                <img width="100%" src="/images/logo/novinki.png">
            </div>
            <div class="col-2-3 about_text">
                Новинки каждый день
            </div>
        </div>
    </div>
    <div class="col-1-5" style="margin-top: 10px">
        <div class="row">
            <div class="col-1-3">
                <img width="100%" src="/images/logo/minorder.png">
            </div>
            <div class="col-2-3 about_text">
                Минимальный заказ всего от 5000 рублей
            </div>
        </div>
    </div>
</div>