    ...
    $(document).on('click','#prod-info',function(){
       ...
       console.log(data);
        ...
        $.post('/site/product',{id: dp},function(data){
            ...
            $size_html+='<div class="size-block" style="overflow: hidden;margin-bottom: 38px; width: 460px;">';
            $baseduri=window.location.hostname;
            if(data['product']['productsAttributesDescr']!=0){
                $size_arr=data['product']['productsAttributesDescr'];
                $size_arr.forEach(function(item,i,arr){
                    console.log(item);
