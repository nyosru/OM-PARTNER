...
if ($data[0] != 'Не найдено!') {
    ...
    foreach ($countdisp as $key => $countdisp) {
        ...
        $innercount .= '<div class="count lock-on"> <a data-pjax="1" class="' . $classcount . '" onclick=""  data-count="' . $countdisp . '"  href="' . $url::toRoute($paste). '">' . $countdisp . '</a></div>';
    }
    ...
    if ($catpath['num'] != 0) {
        foreach ($catpath['num'] as $key => $catid) {
            $paste = $urlsrc;
            $paste['page'] = 1;
            echo '<a data-pjax="1" href="' . $url::toRoute($paste) . '" class="lock-on">';
    ...
    $headbside .=
        '<div class="filter-cart" style="padding: 9px 9px; float: right; text-align: right;"><div style="background: rgb(255, 191, 8) none repeat scroll 0% 0%; font-size: 12px; float: right; position: relative; right: 35px;" class="selected-count badge"></div><a data-pjax="1" class="top-link" href="/glavnaya/selectedproduct"><i class="fa fa-star" style="font-size: 28px; color: rgb(0, 165, 161);"></i></a></div>
                    <div class="filter-cart" style="padding: 9px 9px; float: right; text-align: right;"><div style="background: rgb(255, 191, 8) none repeat scroll 0% 0%; font-size: 12px; float: right; position: relative; right: 35px;" class="cart-count badge"></div><a data-pjax="1" class="top-link" href="/glavnaya/cart"><i class="fa fa-shopping-cart" style="font-size: 28px; color: rgb(0, 165, 161);"></i></a></div>';

    $headbside .='<div class="filter-auth" style="float: right; width: 25%; padding: 14px; font-size: 14px; font-weight: 300;">';

    if(Yii::$app->user->isGuest){
        $headbside .='<div style="float: right; line-height: 2;"><i class="mdi" style="color: rgb(254, 213, 23); font-size: 24px; float: left; line-height: 0.9;">&#xE7FF;</i><a data-pjax="1" data-toggle="modal" style="float: left; cursor:pointer;" data-target="#authform">Вход</a>';
        $headbside .= '</div>';
        $headbside .= '<div style="float: right;"><a data-pjax="1" href="'.BASEURL.'/signup" ><span style="float: left; margin: 4px;">Регистрация</span></a></div>';
    }else{
        $headbside .= '<div style="float: right;"><a data-pjax="1" href="'.BASEURL.'/logout" data-method="post"><i class="mdi" style="color: rgb(254, 213, 23); font-size: 24px; float: left;">&#xE879;</i><span style="float: left; margin: 4px;">Выход</span></a></div>';
        $headbside .= '<div style="float: right;"><a data-pjax="1" href="'.BASEURL.'/lk"><i class="mdi" style="color: rgb(254, 213, 23); font-size: 24px; float: left;">&#xE7FF;</i><span style="float: left; margin: 4px;">Профиль</span></a></div>';
    }
    $headbside .=   '</div>
                         <a data-pjax="1" class="collapsed"  role="button" data-toggle="collapse" data-parent="#accordion" aria-expanded="false" aria-controls="collapseOne">
    ...
    if(count($data[3])>1){
        ...
        foreach($data[3] as $key=>$value){
            ...
            if($value['products_options_values_id']) {
                ...
                $headbside .= '
                        <div class="checkboxmty-overlay fa' . $checked . '" for="checkbox-hidden-group">' .
    ...
    $headbside .= '<a data-pjax="1" href="'.BASEURL.'/changecardview" style="float: right; color: rgb(0, 165, 161); margin-right: 30px; font-size: 16px; border: 1px solid rgb(204, 204, 204); padding: 0px 25px; border-radius: 4px; font-weight: 500;" class="lock-on">Вид</a>';

    $headbside .= ' <a data-pjax="1" href="#demo" style="float: left; color: rgb(0, 165, 161); margin-right: 30px; font-size: 16px; border: 1px solid rgb(204, 204, 204); border-radius: 4px; font-weight: 500; padding: 0px 25px; text-align: center; width: 200px;" data-toggle="collapse">Сортировка</a>';

    $absok = abs($ok-1);
    $paste = $urlsrc;
    $paste['ok'] = $absok;
    $paste['page'] = 1;
    $headbside .= ' <a  data-pjax="1" href="' .  $url::toRoute($paste). '" style="float: left; color: rgb(0, 165, 161); margin-right: 30px; font-size: 16px; border: 1px solid rgb(204, 204, 204); border-radius: 4px; font-weight: 500; padding: 0px 25px; text-align: center; width: 200px;" class="lock-on">ОК</a>';
    ...
    $paste['lux'] = $abslux;
    $paste['page'] = 1;
    $headbside .= ' <a data-pjax="1" href="' .  $url::toRoute($paste). '" style="float: left; color: rgb(0, 165, 161); margin-right: 30px; font-size: 16px; border: 1px solid rgb(204, 204, 204); border-radius: 4px; font-weight: 500; padding: 0px 25px; text-align: center; width: 200px;" class="lock-on">LUX</a>';
    ...
    foreach ($sortorder as $value) {
        ...
        if ($value[1] == $data[11] || $value[2] == $data[11]) {
            ...
            $headbside .= '<a data-pjax="1" class="' . $class . ' lock-on" href="' .  $url::toRoute($paste). '" data="' . $dataord . '" href="#"><div class="header-sort-item-'.$value[3].' header-sort-item active lock-on">'. $value[0] . ' <i style="float: right; padding: 3px 10px;" class="fa fa-' . $arrow . '"> </i></div></a>';
        } else {
            $paste = $urlsrc;
            $paste['sort'] = $dataord;
            $paste['page'] = 1;
            $headbside .= '<a data-pjax="1" class="' . $class . ' lock-on" data="' . $dataord . '" href="' .   $url::toRoute($paste). '"><div class="header-sort-item-'.$value[3].' header-sort-item lock-on">' . $value[0] . '</div></a>';
        }
    }
    ...
    if($data[1] > $count){
        ...
        $paste = $urlsrc;
        $paste['page'] = 1;
        echo '<a data-pjax="1" href="' .  $url::toRoute($paste)  . '" data-page="1" class="lock-on">';
        echo 'Первая';
        echo '</a>';
        echo '</li>';
        echo '<li class="prev">';
        ...
        echo '<a data-pjax="1" href="' .  $url::toRoute($paste) . '" data-page="'.($page-1).'" class="lock-on">';
        ...
        for($startpage; $startpage<=$endpage ; $startpage++){
            if($page == $startpage){
                ...
                echo '<li class="active"><a data-pjax="1" href="' . $url::toRoute($paste) . '" data-page="'.($startpage).'">'.($startpage).'</a></li>';
            }else{
                $paste = $urlsrc;
                $paste['page'] = max(1,$startpage);
                echo '<li><a data-pjax="1" href="' .  $url::toRoute($paste) . '" class="lock-on">'.($startpage).'</a></li>';
            }
        }
        echo '<li class="next">';
        $paste = $urlsrc;
