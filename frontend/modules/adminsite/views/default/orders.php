        ...
        ['class' => 'yii\grid\ActionColumn',
            ...
            'buttons' => [
                'delegate' => function ($url, $model, $key) {
                    if ($model->status == '1' && !($model->orders_id > 0)) {
                        ...
                        $modal .= '<div>Подтвердите отправку  заказа № ' .
                            ...
                            '<a href="/adminsite/default/delegate?id=' . $key . '&self=' . Yii::$app->params['partnersset']['transport']['active'] . '" class="" style="margin: 20px; background: green none repeat scroll 0% 0%; padding: 5px 10px; border-radius: 3px;cursor:pointer;">Да</a>' .
                ...
                'cancel' => function ($url, $model, $key) {
                    if ($model->status == 1 && !($model->orders_id > 0)) {
                        ...
                        $modal .= '<div>Вы уверены что хотите отменить заказ  № ' .
                            ...
                            '<a href="/adminsite/default/cancelorder?id=' . $key . '" class="" style="margin: 20px; background: green none repeat scroll 0% 0%; padding: 5px 10px; border-radius: 3px;cursor:pointer;">Да</a>' .
                            '<a class="" data-dismiss="modal" aria-hidden="true"  style="padding: 5px 10px; border-radius: 3px; margin: 20px; background: red none repeat scroll 0% 0%; cursor:pointer;">Нет</a></div>';
                        $modal .= '</div></div></div></div>';
                ...
                'edit' => function ($url, $model, $key) {
                    if ($model->status == 1 && !($model->orders_id > 0)) {
                        $url = Yii::$app->urlManager->createUrl(['/adminsite/default/orderupdate', 'id' => $key]);
                        return '<div class="col-md-2">' . Html::a(
                ...
                'revert' => function ($url, $model, $key) {
                    if ($model->orders_id > 2) {
                        $url = Yii::$app->urlManager->createUrl(['/adminsite/default/orderrevert', 'id' => $key]);
                        return '<div class="col-md-2">' . Html::a(
...
<script>
    $(document).on('click', '#gendoc', function () {
        ...
        $.ajax({
            url: "/adminsite/default/documents",
            ...
            success: function (data) {
                $('#docpre' + $id).html('<div style="margin: 10px 0px;">' +
                    '<a style="background: rgb(255, 255, 255) none repeat scroll 0% 0%; margin: 0px 10px;" href="/adminsite/default/documents?id=' + $id + '&type=' + $typedoc + '&doc=' + $doc + '&action=load">Загрузить</a>' +
                    '<a style="background: rgb(255, 255, 255) none repeat scroll 0% 0%; margin: 0px 10px;" href="/adminsite/default/documents?id=' + $id + '&type=' + $typedoc + '&doc=' + $doc + '&action=gen" target="_blanK">Открыть в новом окне</a>' +
                    '<a style="background: rgb(255, 255, 255) none repeat scroll 0% 0%; margin: 0px 10px;" href="/adminsite/default/documents?id=' + $id + '&type=' + $typedoc + '&doc=' + $doc + '&action=senduser">Отправить клиенту</a>' +
                    '<a style="background: rgb(255, 255, 255) none repeat scroll 0% 0%; margin: 0px 10px;" href="/adminsite/default/documents?id=' + $id + '&type=' + $typedoc + '&doc=' + $doc + '&action=sendself">Отправить себе на почту</a>' +
                    '<a style="background: rgb(255, 255, 255) none repeat scroll 0% 0%; margin: 0px 10px;" href="/adminsite/default/documents?id=' + $id + '&type=' + $typedoc + '&doc=' + $doc + '&action=print">Отправить себе на почту</a>' +
