...
trait Brands
{
    public function findBrands($limit)
    {
        ...
        $query = Specifications::find()
            ->select([
                'specification_values_description.specification_values_id',
                'specification_values_description.specification_value',
                'COUNT(products_specifications.specification_values_id) AS countProduct'
            ])
            ->joinWith(['specificationValuesDescription','productsSpecifications'])
            ->where(['specification_values.specifications_id'=>'77']);
        if(!empty($brandsallow)) {
            $query->andWhere(['specification_values.specification_values_id' => $brandsallow]);
        }
        if(!empty($unsetbrands)) {
            $query->andWhere(['not in', 'specification_values.specification_values_id', $unsetbrands]);
        }
        if(!empty($limit)){
            $query->limit($limit);
        }
        $brands = $query->groupBy('specification_values_description.specification_values_id')
