...
trait PreCheckProductsToOrder
{
    public function preCheckProductsToOrder($product, $category = FALSE, $region = FALSE, $address_id = FALSE, $count = FALSE, $attr = FALSE)
    {
        ...
        if(!is_array($product)){
            ...
            $product = PartnersProducts::find()->where(['products.`products_id`' => (integer)$product])->JoinWith('productsDescription')->JoinWith('categories')->JoinWith('productsAttributes')->JoinWith('productsAttributesDescr')->andWhere('products_status = 1 and death_reason IS NOT NULL and products.products_quantity > 0 and  products.products_price != 0 ')->asArray()->one();
        }
